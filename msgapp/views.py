...
def homeproc(request):
    ...
    response.write("<h1>这里是首页，具体功能访问<a href='./msggate'>这里</a></h1>")
...
def homeproc2(request):
    ...
    response = FileResponse(open(cwd + "/IMG_4291.JPG","rb"))
    response['Content-Type'] = 'application/octet-stream'
    response['Content-Disposition'] = 'attachment;filename="IMG_4291.JPG"'
    return response

def pgproc(requset):
    template = Template("<h1>这个程序的名字是{{name}}</h1>")
    context = Context({"name" : "实验室" })
